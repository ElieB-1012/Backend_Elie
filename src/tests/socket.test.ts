...
describe("my awesome project", () => {
    ...
    test("postAdd", (done) => {
        client1.socket.emit('post:add_new', { 'message': 'this is my message', 'senderId': client1.id, 'senderName': userEmail, 'photo': '' })
    ...
    test("Test get messages", (done) => {
        ...
        client1.socket.once('chat:message', (args) => {
            expect(args[args.length - 1].to).toBe(client2.id)
            expect(args[args.length - 1].message).toBe(message)
            expect(args[args.length - 1].from).toBe(client1.id)
            done()
        })
