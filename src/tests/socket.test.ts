...
describe("my awesome project", () => {
   ...
   test("postAdd", (done) => {
    client1.socket.emit('post:add_new', { 'message': 'this is my message', 'sender': client1.id })
    client1.socket.on('post:add_new.response', (arg) => {
   ...
   test("Post get all test", (done) => {
    ...
    client1.socket.on('post:get_all.response', (arg) => {
    ...
    client1.socket.on('post:get_by_id.response', (arg) => {
        expect(arg.sender).toBe(client1.id)
        expect(arg.message).toBe('this is my message')
        done()
    })
    ...
    test("Post get by sender", (done) => {
        client1.socket.emit('post:get_by_sender', { 'sender': client1.id })
        client1.socket.on('post:get_by_sender.response', (arg) => {
            expect(arg[0].sender).toBe(client1.id)
            expect(arg[0].message).toBe('this is my message')
            done()
        })
    ...
    client1.socket.emit('post:update_post', { 'body':{ 'message' : 'this is the new test post message'}, 'params': {'id' : newPostId} })
    client1.socket.on('post:update_post.response', (arg) => {
